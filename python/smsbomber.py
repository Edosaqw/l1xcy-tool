...
def watermark():
    print("L1xcy Tool -  İyi kullanımlar <3")

def get_number():
    global clear
    while True:
        try:
            number = int(input(f""" L1xcy Tool - Başına kod koymadan yazınız! örnek: "54xxxxxxxx" (Sadece Türkiye numaralarında çalışır!)\n[?] : """))
            if len(str(number)) == 10 and str(number)[0] == "5":
                return number
            else:
                clear()
                print(f"L1xcy Tool - Yanlış bir numara girildi!")
        except:
            clear()
            print(f"L1xcy Tool - Numara kısmı boş bırakılamaz.")

def get_amount():
    global clear
    while True:
        try:
            amount = int(input(f"""L1xcy Tool - Lütfen SMS sayısı belirtin. (sınırsız gönderim için "0")\n[?] : """))
            if amount >= 0:
                return amount
            else:
                clear()
                print(f"L1xcy Tool - Geçersiz sayı.")
        except:
            clear()
            print(f"L1xcy Tool - Bu alan boş bırakılamaz.")

def get_worker_amount():
    global clear
    while True:
        try:
            worker_amount = int(input(f"L1xcy Tool - Thread sayısı yazınız. Tavsiye 1-50 Arasıdır. (max 100-200)\n[?] : "))
            if worker_amount >= 1:
                return worker_amount
            else:
                clear()
                print(f"L1xcy Tool - Daha yüksek bir sayı giriniz!")
        except:
            clear()
            print(f"L1xcy Tool - Daha yüksek bir sayı giriniz!.")
...
def restart():
    global clear
    while True:
        question = input(f"Program Kapatılsın mı?\n[Y/N] : ").upper().replace(" ", "")
        if question == "Y":
            quit()
        elif question == "N":
            clear()
